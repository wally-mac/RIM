...
from lib.shapefile import create_field
from lib.shapefile import get_transform_from_epsg
import json
...
def export_table(filegdb, tablename, db_path, retained_fields, attribute_filter):
    ...
    for in_feature in in_layer:
        ...
        sql = 'INSERT INTO {0} (FID, {1}) VALUES ({2}, {3})'.format(tablename, ','.join([name for name, dtype, idx in header]), counter, ','.join(['?' for val in header]))
...
def main():
    ...
    parser.add_argument('filegdb', help='File geodatabase path', type=str)
    parser.add_argument('featureclass', help='File geodatabase feature class to be exported', type=str)
    parser.add_argument('output_dir', help='Directory where Shapefile will be created', type=str)
    parser.add_argument('epsg', help='EPSG of the output Shapefile', type=int)
    parser.add_argument('--retained_fields', help='Comma separated list of fields to kept. All if omitted.', type=str)
    args = parser.parse_args()

    # filegdb, featureclass, output_dir, output_epsg, retained_fields, attribute_filter, spatial_filter
    export_feature_class(args.filegdb, args.featureclass, args.output_dir, args.epsg, args.retained_fields, None, None)
