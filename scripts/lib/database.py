...
from build_network import FCodeValues
...
def create_database(huc, db_path, metadata, epsg):
    ...
    schema_path = os.path.join(os.path.abspath(os.path.dirname(__file__)), '../database/brat_schema.sql')
    ...
    for dirName, dirs, files in os.walk(os.path.join(os.path.abspath(os.path.dirname(__file__)), '../database/data')):
        for file in files:
